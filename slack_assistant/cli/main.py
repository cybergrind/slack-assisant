...
# Suppress verbose HTTP logging from httpx (used by anthropic SDK)
...
@cli.command('status-agent')
def status_agent():
    """Start interactive status agent."""
    config = get_config()
    errors = config.validate()
    if errors:
        for error in errors:
    ...
    async def run_agent():
        from slack_assistant.agent import AgentController
